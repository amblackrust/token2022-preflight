...
function report(
  status: PreflightReport["overallStatus"] = "READY",
): PreflightReport {
  return {
    schemaVersion: "1.0",
    engineVersion: "0.1.0",
...
describe("API", () => {
  ...
  it("rejects invalid input before invoking the analyzer", async () => {
    ...
    const response = await app({ analyzer }).inject({
      method: "POST",
      url: "/v1/preflight",
      payload: { cluster: "testnet", mint: "bad" },
    });
  ...
  it("maps a missing mint to HTTP 404", async () => {
    const response = await app({
      analyzer: async () => {
        throw new PreflightError("ACCOUNT_NOT_FOUND", "not found");
      },
    }).inject({
      method: "POST",
      url: "/v1/preflight",
