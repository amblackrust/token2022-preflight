...
export type Analyze = (input: AnalyzeTokenTransferInput) => Promise<PreflightReport>;
...
export async function runCli(
  ...
  program.command('inspect')
    ...
    .addOption(new Option('--cluster <cluster>').choices(['mainnet-beta', 'devnet']).default('mainnet-beta'))
    ...
    .action(async (mint: string, options: InspectOptions) => {
      try {
        ...
        const rpcUrl = options.rpcUrl ?? env.SOLANA_RPC_URL ?? DEFAULT_RPC_URLS[options.cluster];
        const input: AnalyzeTokenTransferInput = {
          cluster: options.cluster,
          rpcUrl,
          mint,
          ...(options.amount === undefined ? {} : { amountUi: options.amount }),
          ...(options.source === undefined ? {} : { sourceTokenAccount: options.source }),
          ...(options.destination === undefined ? {} : { destinationTokenAccount: options.destination }),
        };
        const report = await analyze(input);
        io.writeStdout(options.json ? `${JSON.stringify(report, null, 2)}\n` : formatTerminalReport(report, options.color));
...
export function formatTerminalReport(report: PreflightReport, color: boolean): string {
  const paint = color ? statusColor(report.overallStatus) : (value: string) => value;
  const lines = [
  ...
  if (report.transfer !== undefined) {
    lines.push('', 'Transfer', `  Send       ${report.transfer.amountRaw ?? 'unknown'}`);
    if (report.transfer.expectedFeeRaw !== undefined) lines.push(`  Fee        ${report.transfer.expectedFeeRaw}`);
    if (report.transfer.expectedReceivedRaw !== undefined) lines.push(`  Receive    ${report.transfer.expectedReceivedRaw}`);
  }
  lines.push('', 'Findings');
  if (report.findings.length === 0) lines.push('  READY   No blockers found by supported checks');
...
function validateTimeout(value: string): void {
  if (!/^\d+$/.test(value) || Number(value) < 1) {
    throw new PreflightError('UNEXPECTED_ERROR', 'Timeout must be a positive integer');
  }
}
...
function statusColor(status: FindingStatus): (value: string) => string {
  switch (status) {
    ...
    case 'ACTION_REQUIRED': return pc.yellow;
    case 'UNKNOWN': return pc.magenta;
    case 'WARNING': return pc.yellow;
    case 'READY': return pc.green;
  }
}
