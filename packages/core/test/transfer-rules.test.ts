...
import { analyzeNormalizedToken, type NormalizedAnalysis } from '../src/index.js';
...
describe('transfer-aware rules', () => {
  it('calculates the epoch-selected transfer fee with a maximum cap', () => {
    const report = analyzeNormalizedToken(analysis({
      amountUi: '1000',
      currentEpoch: 42n,
      mint: {
        ...analysis().mint,
  ...
  it('blocks a paused mint and reports its authority', () => {
    const report = analyzeNormalizedToken(analysis({
      mint: {
        ...analysis().mint,
        extensions: [{ kind: 'PausableConfig', paused: true, authority: 'Pause1111111111111111111111111111111111111' }],
      },
    }));

    expect(report.overallStatus).toBe('BLOCKED');
    expect(report.findings[0]).toMatchObject({ id: 'mint-paused', status: 'BLOCKED' });
  });
  ...
  it('warns for default-frozen state without claiming existing accounts are frozen', () => {
    ...
    expect(report.findings[0]).toMatchObject({ id: 'default-account-state-frozen', status: 'WARNING' });
  });

  it('blocks a frozen source and requires a memo for the destination', () => {
    const report = analyzeNormalizedToken(analysis({
      sourceTokenAccount: { address: SOURCE, state: 'frozen', extensions: [] },
      destinationTokenAccount: {
        address: DESTINATION,
        state: 'initialized',
        extensions: [{ kind: 'MemoTransfer', requireIncomingTransferMemos: true }],
      },
    }));

    expect(report.overallStatus).toBe('BLOCKED');
    expect(report.findings.map(({ id }) => id)).toEqual(['source-frozen', 'destination-memo-required']);
  });
    ...
    const report = analyzeNormalizedToken(analysis({
      mint: {
        ...
        extensions: [{
          ...
          resolution: { status: 'unresolved', reason: 'meta account not found' },
        }],
      },
    }));

    expect(report.findings.map(({ id }) => id)).toEqual(['transfer-hook', 'transfer-hook-unresolved']);
  });
  ...
  it('warns about a permanent delegate and marks confidential transfer unsupported', () => {
    const report = analyzeNormalizedToken(analysis({
      mint: {
        ...analysis().mint,
        extensions: [
          { kind: 'PermanentDelegate', delegate: 'Delegate11111111111111111111111111111111111' },
    ...
    expect(report.findings.map(({ status }) => status)).toEqual(['UNKNOWN', 'WARNING']);
  });
});
